...
from distribution import StandardGaussian, GeneralGaussian, LinftyGaussian, LinftyGeneralGaussian, L1GeneralGaussian, ExGeneralGaussian, ExStandardGaussian
...
parser.add_argument("eta", type=float, default=2.0, help="power in general gaussian distribution")
parser.add_argument("--disttype", type=str, help="smoothing distribution type", choices=['gaussian', 'general-gaussian',
                                                                                         ...
                                                                                         'L1-general-gaussian', 'exponential-standard-gaussian', 
...
parser.add_argument('--exth', action='store_true', help='the threshholds will be demtermined by respective exponential distributions')
...
if __name__ == '__main__':
    ...
    file_name = f'{args.disttype}-{args.k}-{args.std}-{args.eta}-{args.N}-{args.alpha}'
    ...
    if args.exth:
        writer = SummaryWriter(os.path.join(out_dir, f'{args.disttype}-{args.k}-{args.std}-{args.eta}-{args.N}-{args.alpha}-exth'))
    else:
        writer = SummaryWriter(os.path.join(out_dir, f'{args.disttype}-{args.k}-{args.std}-{args.eta}-{args.N}-{args.alpha}'))
    ...
    elif args.disttype == 'exponential-standard-gaussian':
        distribution = ExStandardGaussian(d, args.std, args.eta, args.exth, th=args.th if isinstance(args.th, float) else 1.0)
    elif args.disttype == 'exponential-general-gaussian':
        distribution = ExGeneralGaussian(d, args.k, args.std, args.eta, args.exth, th=args.th if isinstance(args.th, float) else 1.0)
...
""", file=f)
    ...
    if isinstance(args.th, str):
        ...
        old_pas = dict()
        old_pars = dict()
        old_file_name = f'{args.disttype}-{args.k}-{args.std}-{args.eta}-{args.N}-{args.alpha}'
        # if args.exth:
        #     old_file_name += '-exth'
        old_file_name += '.txt'
    ...
    for i in range(len(dataset)):
        ...
        if args.th == 'x+':
            if old_pars[i] >= 1.0 - 1e-8:
                print(f'old pars is {old_pars[i]}')
                print(f'old pas is {old_pas[i]}')
        ...
        # print(distribution, num_classes)
