...
from distribution import StandardGaussian, GeneralGaussian, LinftyGaussian, LinftyGeneralGaussian, \
        L1GeneralGaussian, ExStandardGaussian, ExGeneralGaussian
...
def calc_orig_radius(base_dir, out_base_dir, disttype, d, k, std, eta, aux_stds, N, alpha):
    """
    ...
    if disttype == 'gaussian' or disttype == 'gaussian-th':
        ...
        dist = ExStandardGaussian(d, std, eta, args.exth)
        ...
        dist = ExGeneralGaussian(d, k, std, eta, args.exth)
    ...
    if disttype == 'gaussian' or disttype == 'gaussian-th':
        ...
        fname = f'exponential-standard-gaussian-None-{std}-{eta}-{N}-{alpha}.txt'
        outname = f'orig-rad-{disttype}-None-{std}-{eta}-{N}-{alpha}.txt'
        timename = 'time-' + outname
        f = open(os.path.join(out_base_dir, outname), 'a')
        ftime = open(os.path.join(out_base_dir, timename), 'a')
        writer = SummaryWriter(os.path.join(out_base_dir, f'orig-rad-{disttype}-None-{std}-{eta}-{N}-{alpha}'))
        pAlist = read_pAs(os.path.join(base_dir, fname))
        res = Pool(workers).map(orig_radius_pool_func, pAlist)
        for i in range(len(res)):
            print('On #', pAlist[i][0])
        ...
        samp_disttype = 'exponential-general-gaussian'
        fname = f'{samp_disttype}-{k}-{std}-{eta}-{N}-{alpha}.txt'
        outname = f'orig-rad-{disttype}-{k}-{std}-{eta}-{N}-{alpha}.txt'
        timename = 'time-' + outname
        f = open(os.path.join(out_base_dir, outname), 'a')
        ftime = open(os.path.join(out_base_dir, timename), 'a')
        writer = SummaryWriter(os.path.join(out_base_dir, f'orig-rad-{disttype}-{k}-{std}-{eta}-{N}-{alpha}'))
...
def combine_info(sampling_dir, orig_rad_dir, disttype, d, k, std, eta, aux_stds, N, alpha):
    """
        info list format:
    ...
    if disttype == 'exponential-general-gaussian' or disttype == 'exponential-standard-gaussian':
        orig_rad_fname = f'orig-rad-{disttype}-{k}-{std}-{eta}-{N}-{alpha}.txt'
        info_list = read_orig_Rs(os.path.join(orig_rad_dir, orig_rad_fname), aux_stds)
        index_mapper = dict()
        for i, item in enumerate(info_list):
            index_mapper[item[0]] = i

        main_pA_fname = f'{disttype}-{k}-{std}-{eta}-{N}-{alpha}.txt'
        data = read_pAs(os.path.join(sampling_dir, main_pA_fname))
        for item in data:
            if item[0] in index_mapper:
                # p1low
        ...
        for i, aux_std in enumerate(aux_stds):
            if args.exth:
                aux_pA_fname = f'{disttype}-{k}-{aux_std}-{eta}-{N}-{alpha}-exth.txt'
            else:
                aux_pA_fname = f'{disttype}-{k}-{aux_std}-{eta}-{N}-{alpha}.txt'
            data = read_pAs(os.path.join(sampling_dir, aux_pA_fname))
            for item in data:
                if item[0] in index_mapper:
                    info_list[index_mapper[item[0]]][4][i][0], info_list[index_mapper[item[0]]][4][i][1] = item[1], item[2]
    ...
    elif disttype == 'exponential-general-gaussian-th' or disttype == 'exponential-standard-gaussian-th':
        orig_rad_fname = f'orig-rad-{disttype[:-3]}-{k}-{std}-{eta}-{N}-{alpha}.txt'
        info_list = read_orig_Rs(os.path.join(orig_rad_dir, orig_rad_fname), aux_stds)
        index_mapper = dict()
        for i, item in enumerate(info_list):
            index_mapper[item[0]] = i

        main_pA_fname = f'{disttype[:-3]}-{k}-{std}-{eta}-{N}-{alpha}.txt'
        data = read_pAs(os.path.join(sampling_dir, main_pA_fname))
        for item in data:
            if item[0] in index_mapper:
                # p1low
        ...
        for i, aux_std in enumerate(aux_stds):
            if args.exth:
                aux_pA_fname = f'{disttype[:-3]}-{k}-{std}-{eta}-{N}-{alpha}-{aux_std}-exth.txt'
            else:
                aux_pA_fname = f'{disttype[:-3]}-{k}-{std}-{eta}-{N}-{alpha}-{aux_std}.txt'
            data = read_pAs(os.path.join(sampling_dir, aux_pA_fname))
            for item in data:
                if item[0] in index_mapper:
                    info_list[index_mapper[item[0]]][4][i][0], info_list[index_mapper[item[0]]][4][i][1] = item[1], item[2]
...
def new_radius_pool_func(args):
    ...
    # print('get LIST :', no, orig_r, pAsigmaL, pAsigmaR, pAbetaL, pAbetaR)
    print('On #', no)
    stime = time()
            ...
            while True:
                ...
                try:
                    ...
                    if check(bunk_disttype, slot * bunk_radius_unit,
                             bunk_radius_d, bunk_radius_k, bunk_radius_sigma, bunk_radius_eta, bunk_radius_beta,
            ...
            if bunk_radius_mode == 'precise':
                ...
                while rad_R - rad_L > bunk_radius_eps:
                    ...
                    if res:
                        ...
                        rad_R = mid
                new_r = rad_L
    print(f'Result on #{no} (sigma={bunk_radius_sigma}, eta={bunk_radius_eta}, beta={bunk_radius_beta}) R = {orig_r} + {new_r - orig_r} [time={time() - stime} s]')
    runtime = time() - stime
...
def bunk_radius_calc(full_info, result_dir, disttype, d, k, sigma, eta, betas, N, alpha, mode, unit, eps):
    """
        ...
        :param eta: power for gaussian-like distribution
        ...
        :return: None
    """
    global bunk_disttype, bunk_radius_d, bunk_radius_k, bunk_radius_sigma, bunk_radius_eta, bunk_radius_beta
    ...
    bunk_radius_d, bunk_radius_k, bunk_radius_eta = d, k, eta
    ...
    if bunk_disttype == 'general-gaussian' or bunk_disttype == 'gaussian' \
            ...
            or bunk_disttype == 'exponential-general-gaussian' or bunk_disttype == 'exponential-standard-gaussian' \
            or bunk_disttype == 'exponential-general-gaussian-th' or bunk_disttype == 'exponential-standard-gaussian-th':
        for i, beta in enumerate(betas):
            ...
            # the beta values for exponential gaussian distributions are never used 
            elif bunk_disttype == 'exponential-general-gaussian':
                bunk_radius_sigma = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma((d - 2 * k)/ eta) - lngamma((d - 2 * k + 2) / eta))) * sigma
                bunk_radius_beta = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma((d - 2 * k)/ eta) - lngamma((d - 2 * k + 2) / eta))) * beta
            elif bunk_disttype == 'exponential-general-gaussian-th':
                bunk_radius_sigma = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma((d - 2 * k)/ eta) - lngamma((d - 2 * k + 2) / eta))) * sigma
                bunk_radius_beta = beta
            elif bunk_disttype == 'exponential-standard-gaussian':
                bunk_radius_sigma = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma(d / eta) - lngamma((d + 2) / eta))) * sigma
                bunk_radius_beta = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma(d / eta) - lngamma((d + 2) / eta))) * beta
            elif bunk_disttype == 'exponential-standard-gaussian-th':
                bunk_radius_sigma = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma(d / eta) - lngamma((d + 2) / eta))) * sigma
                bunk_radius_beta = beta
            else:
                ...
                bunk_radius_beta = beta
            if args.exth:
                fname = f'new-rad-{disttype}-{k}-{sigma}-{eta}-{beta}-{N}-{alpha}-exth.txt'
            else:
                fname = f'new-rad-{disttype}-{k}-{sigma}-{eta}-{beta}-{N}-{alpha}.txt'
            ftimename = 'time-' + fname
            f = open(os.path.join(result_dir, fname), 'a')
            ftime = open(os.path.join(result_dir, ftimename), 'a')
            bunk_global_writer = GlobalSummaryWriter(os.path.join(result_dir, f'new-rad-{disttype}-{k}-{sigma}-{eta}-{beta}-{N}-{alpha}'))
...
parser.add_argument('task', choices=['origin', 'improved'],
                    help='The workflow is to first use the classical Nayman-Pearson certification to compute the radius -- choices = origin,'
                                'then use the DSRS to compute the improved radius -- choices = improved.'
                                'They are separate runs')
parser.add_argument('model', type=str,
                    help='We load the sampling probability and old radius from the current folder: sampling_dir/model, original_rad_dir/model,'
...
parser.add_argument('disttype', choices=['general-gaussian', 'gaussian',
                                         ...
                                         'exponential-general-gaussian', 'exponential-standard-gaussian', 
                                         'exponential-general-gaussian-th', 'exponential-standard-gaussian-th'],
...
parser.add_argument('eta', type=float, default=2.0,
                    help="power of P distribution.")    
parser.add_argument('--exth', action='store_true', help="whether to use varied power threshold")                    
...
parser.add_argument("--k", type=int, default=None,
                    help="the generalized Gaussian's parameter k. See the paper for the detail")
...
parser.add_argument('--new_rad_dir', type=str, default='data/new-radius',
                    help="folder for outputing the new radius compuated by DSRS")

parser.add_argument('--improve_mode', choices=['grid', 'fast', 'precise'], default='grid',
                    help='the strategy for trying the new radius. Empirically, the grid strategy is sufficient if we do not need the precise new radius.')
...
parser.add_argument('--workers', type=int, default=10,
                    help='# processes for parallelized mapping on CPU.')
...
""")
    ...
    if args.task == 'origin':
        ...
        calc_orig_radius(os.path.join(args.sampling_dir, args.model), os.path.join(args.original_rad_dir, args.model),
                         args.disttype, d, k, sigma, eta, betas, N, alpha)
