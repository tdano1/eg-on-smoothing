...
import numpy as np
from numpy import linalg, ubyte
import torch
import time
...
def Int_for_gamma(func, d, eta, logK, cons=1e-4, num_dif=256):
    iota = np.sqrt(eta / cons) / np.sqrt(d)
    lb = (1 - iota) * d / eta
    lb = 0 if lb < 0 else lb
    ub = (1 + iota) * d / eta
    dif = (ub - lb) / num_dif
    Int = 0
    for i in range(int(num_dif / 2)):
        Int += gamma.pdf(lb + i * dif, d / eta) * func(lb + i * dif, logK) * dif
        Int += gamma.pdf(ub - (i + 1) * dif, d / eta) * func(ub - (i + 1) * dif, logK) * dif
    return Int

def u2x(u, sigma, eta):
    '''return ||x||_2 = sigma * (2 * u) ** (1 / eta)'''
    return sigma * (2.0 * u) ** (1 / eta)

def x2u(x, sigma, eta):
    '''return u = 1/2 * (x / sigma) ** eta'''
    return 1 / 2 * (x / sigma) ** eta 

def inv2_phig(x, logK, k, sigma, eta):
    '''return inv_phi_g^2(x)'''
    ans = sigma * sigma * (4.0 * k / eta * lambertWlog(-eta / 2 / k * logK + np.log(eta / 2 / k * x2u(x, sigma, eta))
            + eta / 2 / k * x2u(x, sigma, eta))) ** (2 / eta)
    return ans.real

def inv2_phis(x, logK, sigma, eta):
    '''return inv_phi_s^2(x)'''
    return (2 ** (1 / eta) * sigma * (x2u(x, sigma, eta) - logK) ** (1 / eta)) ** 2
...
def _compute_at_origin_egg(d, k, sigma, eta, logK, rho):
    ans = gamma((d - 2 * k) / eta).expect(
        lambda x: beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
            ((u2x(x, sigma, eta) + rho) ** 2 - inv2_phig(u2x(x, sigma, eta), -logK, k, sigma, eta)) / (4.0 * u2x(x, sigma, eta) * rho)
        ),
        lb=0., ub=np.inf
    )
    return ans

def _compute_with_shift_egg(d, k, sigma, eta, logK, rho):
    ans = 1 - gamma((d - 2 * k) / eta).expect(
        lambda x: beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
            ((u2x(x, sigma, eta)  + rho) ** 2 - inv2_phig(u2x(x, sigma, eta), logK, k, sigma, eta)) / (4.0 * u2x(x, sigma, eta) * rho)
        ),
        lb=0., ub=np.inf
    )
    # print(f'shift egg is {ans}')
    return ans

def _compute_at_origin_esg(d, sigma, eta, logK, rho):
    def get_Beta_val(u, logK):
        if u + logK >= 1e-10 and u >= 1e-10:
            ans = beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(u, sigma, eta)  + rho) ** 2 - inv2_phis(u2x(u, sigma, eta), -logK, sigma, eta)) 
                / 
                (4.0 * u2x(u, sigma, eta) * rho))
        else:
            ans = 1
        return ans
...
def _compute_with_shift_esg(d, sigma, eta, logK, rho):
    def get_Beta_val(u, logK):
        if u - logK >= 1e-10 and u >= 1e-10:
            ans = beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(u, sigma, eta) + rho) ** 2 - inv2_phis(u2x(u, sigma, eta), logK, sigma, eta)) 
...
def _binary_search_logK_egg(d, k, sigma, eta, r, pA):
    logK_lb, logK_ub = -100., 100.
    EPS = 1e-6
    while logK_ub - logK_lb > EPS:
        # print(f'  on {r} binary search logK in [{logK_lb}, {logK_ub}]')
        logK_mid = (logK_lb + logK_ub) / 2.0
        # print(f'logK is {logK_mid}')
        # print(d, sigma, eta, logK_mid, r)
        now_mass = _compute_at_origin_egg(d, k, sigma, eta, logK_mid, r)
        if now_mass < pA:
            logK_ub = logK_mid
        else:
            logK_lb = logK_mid
...
def _binary_search_logK_esg(d, sigma, eta, r, pA):
    logK_lb, logK_ub = -100., 100.
    EPS = 1e-6
    while logK_ub - logK_lb > EPS:
        # print(f'  on {r} binary search logK in [{logK_lb}, {logK_ub}]')
        ...
        # print(f'logK is {logK_mid}')
        # print(d, sigma, eta, logK_mid, r)
        now_mass = _compute_at_origin_esg(d, sigma, eta, logK_mid, r)
        if now_mass < pA:
            logK_ub = logK_mid
        else:
            logK_lb = logK_mid
...
class ExStandardGaussian(Distribution):
    """
        Exponential Standard Gaussian
        proc exp(-||x||_2^eta / (2 sigma^eta))
    """

    def __init__(self, d, scale, eta, exth, eps=1e-6, th=1.0):
        super(ExStandardGaussian, self).__init__(d, scale)
        d = int(d)
        self.sigma = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma(d / eta) - lngamma((d + 2) / eta))) * self.scale
        self.eta = eta
        self.exth = exth
        self.th = th
        self.eps = eps
        ...
        if th < 1.0 - eps:
            ...
            # we need to figure out such threshold
            if self.exth:
                self.thres = gamma(self.d / self.eta).ppf(self.th)
            else:
                self.thres = gamma(self.d / 2).ppf(self.th)
            # print('thres set to:', self.thres)
    ...
    def set_th(self, th):
        if th < 1.0 - self.eps:
            ...
            self.th = th
            if self.exth:
                self.thres = gamma(self.d / self.eta).ppf(th)
            else:
                self.thres = gamma(self.d / 2).ppf(th)
            # print('thres set to:', self.thres)
    ...
    def mean_norm(self):
        return 2 ** (1 / self.eta) * self.sigma * np.sqrt ( np.exp(lngamma((self.d + 2) / self.eta) - lngamma(self.d / self.eta)))

    def info(self):
        return f'Exponential Standard Gaussian distribution with scale {self.scale} and sigma {self.sigma}'
    ...
    def certify_radius(self, pA):
        ...
        if abs(pA - self.pAtable.get(key, 0.0)) < self.eps:
            ...
            while r_ub - r_lb > self.eps:
                ...
                r_mid = (r_lb + r_ub) / 2.0
                # print(self.d, self.sigma, self.eta, r_mid, pA)
                logK = _binary_search_logK_esg(self.d, self.sigma, self.eta, r_mid, pA)
                shifted_pA = _compute_with_shift_esg(self.d, self.sigma, self.eta, logK, r_mid)
                if shifted_pA > 0.5:
                    r_lb = r_mid
                else:
                    r_ub = r_mid
...
class ExGeneralGaussian(Distribution):
    """
        Exponential General Gaussian
        proc ||x||_2^{-2k} exp(-||x||_2^eta / (2 sigma^eta))
    """

    def __init__(self, d, k, scale, eta, exth, eps=1e-6, th=1.0):
        super(ExGeneralGaussian, self).__init__(d, scale)
        ...
        self.eta = eta
        self.exth = exth
        self.sigma = 0.5 ** (1 / eta) * np.sqrt(d * np.exp(lngamma((d - 2 * k)/ eta) - lngamma((d - 2 * k + 2) / eta))) * self.scale
        self.eps = eps
        self.th = th
        self.pAtable = dict()
        self.rTable = dict()
        ...
        if self.th < 1.0 - eps:
            ...
            # we need to figure out such threshold
            if self.exth:
                self.thres = gamma((self.d - 2.0 * self.k) / self.eta).ppf(self.th)
            else:
                self.thres = gamma(self.d / 2.0 - self.k).ppf(self.th)
            # print('thres set to:', self.thres)
    ...
    def set_th(self, th):
        if th < 1.0 - self.eps:
            ...
            self.th = th
            if self.exth:
                self.thres = gamma((self.d - 2 * self.k) / self.eta).ppf(th)
            else:
                self.thres = gamma(self.d / 2.0 - self.k).ppf(th)
            # print('thres set to:', self.thres)
    ...
    def sample(self, batch_size, cuda=False):
        ...
        dist = gamma((self.d - 2 * self.k) / self.eta)
        if self.th >= 1.0 - self.eps:
            r_sq = dist.rvs(batch_size)
        else:
            cnt = 0
    ...
    def mean_norm(self):
        return 2 ** (1 / self.eta) * self.sigma * np.sqrt ( np.exp(lngamma((self.d - 2 * self.k + 2) / self.eta) - lngamma((self.d - 2 * self.k) / self.eta)))

    def info(self):
        return f'Exponential General Gaussian distribution with scale {self.scale} and sigma {self.sigma}'
    ...
    def certify_radius(self, pA):
        ...
        if abs(pA - self.pAtable.get(key, 0.0)) < self.eps:
            ...
            while r_ub - r_lb > self.eps:
                print(f'binary search r in [{r_lb}, {r_ub}]')
                r_mid = (r_lb + r_ub) / 2.0
                logK = _binary_search_logK_egg(self.d, self.k, self.sigma, self.eta, r_mid, pA)
                shifted_pA = _compute_with_shift_egg(self.d, self.k, self.sigma, self.eta, logK, r_mid)
                if shifted_pA > 0.5:
                    r_lb = r_mid
                else:
                    r_ub = r_mid
