...
from distribution import sample_l2_vec, sample_linfty_vec, sample_l1_vec, Int_for_gamma, x2u, u2x, inv2_phis, inv2_phig
from th_heuristic import *
...
def P_single_var_numerical(disttype, d, k, sigma, eta, logK, r):
    """
    ...
    elif disttype == 'exponential-general-gaussian' or disttype == 'exponential-general-gaussian-th':
        ans = Gamma((d - 2 * k) / eta).expect(
            lambda t: Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(t, sigma, eta) + r) ** 2 - inv2_phig(u2x(t, sigma, eta), -logK, k, sigma, eta))
                /
                (4.0 * u2x(t, sigma, eta) * r)
            ),
            lb=0., ub=np.inf
        )
        return ans

    elif disttype == 'exponential-standard-gaussian' or disttype == 'exponential-standard-gaussian-th':
        def get_Beta_val(u, logK):
            if u + logK >= 1e-10 and u >= 1e-10:
                ans = Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    ((u2x(u, sigma, eta)  + r) ** 2 - inv2_phis(u2x(u, sigma, eta), -logK, sigma, eta)) 
                    /
                    (4.0 * u2x(u, sigma, eta) * r))
            else:
                ans = 1
            return ans

        return Int_for_gamma(get_Beta_val, d, eta, logK)
...
def Q_single_var_numerical(disttype, d, k, sigma, eta, beta, logK, r):
    """
    ...
    if disttype == 'gaussian':
    ...
    elif disttype == 'exponential-standard-gaussian-th':
        nu = 1.0 / Gamma(d / eta).cdf(beta)
        ...
        ans = nu * Gamma(d / eta).expect(get_Beta_val, lb=0., ub=beta)
        return ans

    elif disttype == 'exponential-general-gaussian-th':
        nu = 1.0 / Gamma((d - 2 * k) / eta).cdf(beta)
        ans = nu * Gamma((d - 2 * k) / eta).expect(
            lambda t: Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(t, sigma, eta) + r) ** 2 - inv2_phig(u2x(t, sigma, eta), -logK, k, sigma, eta))
                /
                (4.0 * u2x(t, sigma, eta) * r)
    ...
    # elif disttype == 'exponential-general-gaussian':
    #     def inv_gK(x):
    #         ans = sigma * (4.0 * k / eta * 
    #             lambertWlog(eta / 2 / k * logK + np.log(eta / 4 / k * (x / sigma) ** eta) + eta / 4 / k * (x / sigma) ** eta)) ** (1 / eta)
    #         return ans

    #     ans = Gamma((d - 2 * k) / eta).expect(
    #         lambda x: Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
    #             ((sigma * (2.0 * x) ** (1 / eta) + r)**2 - inv_gK(sigma * (2.0 * x) ** (1 / eta))**2) / (4.0 * sigma * (2.0 * x) ** (1 /eta) * r)
    #         ),
    #         lb=0., ub=np.inf
    #     )
    #     return ans

def Pprime_single_var_numerical(disttype, d, k, sigma, eta, beta, logK, r):
    """
    ...
    if disttype == 'gaussian':
        ...
        nu = 1.0 / Gamma(d / eta).cdf(beta)
        def get_Beta_val(u):
            if u + logK - np.log(nu) >= 1e-10 and u >= 1e-10:
                ans = Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    ((u2x(u, sigma, eta)  + r) ** 2 - inv2_phis(u2x(u, sigma, eta), -logK + np.log(nu), sigma, eta)) 
                    /
                     (4.0 * u2x(u, sigma, eta) * r))
            else:
                ans = 1
        ...
        ans = Gamma(d / eta).expect(get_Beta_val, lb=0., ub=beta)
        return ans
    
    elif disttype == 'exponential-general-gaussian-th':
        nu = 1.0 / Gamma((d - 2 * k) / eta).cdf(beta)
        nupow = -eta * np.log(nu) / 2 / k
        ans = Gamma((d - 2 * k) / eta).expect(
            lambda t: Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(t, sigma, eta) + r) ** 2 - inv2_phig(u2x(t, sigma, eta), -logK + np.log(nu), k, sigma, eta))
                /
                (4.0 * u2x(t, sigma, eta) * r)
            ),
            lb=0., ub=beta
...
def Qprime_single_var_numerical(disttype, d, k, sigma, eta, beta, logK, r):
    """
...
def Pshift_single_var_numerical(disttype, d, k, sigma, eta, logK, r, limit=50):
    """
    ...
    if disttype == 'gaussian':
        ...
        ans = 1.0 - Gamma((d - 2 * k) / eta).expect(
            lambda t: Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(t, sigma, eta) + r) ** 2 - inv2_phig(u2x(t, sigma, eta), logK, k, sigma, eta))
                /
                (4.0 * u2x(t, sigma, eta) * r)
        ...
        return 1 - Int_for_gamma(get_Beta_val, d, eta, logK)
...
def bin_search_K_on_P_numerical(disttype, d, k, sigma, eta, rad, P, range=100., EPS=1e-6):
    """
    ...
    while logK_ub - logK_lb > EPS:
        ...
        now_mass = P_single_var_numerical(disttype, d, k, sigma, eta, logK_mid, rad)
        if now_mass < P:
            logK_ub = logK_mid
        else:
            logK_lb = logK_mid
...
def bin_search_K_on_Qprime_numerical(disttype, d, k, sigma, eta, beta, rad, Q, range=20., EPS=1e-6):
    """
    ...
    while logK_ub - logK_lb > EPS:
        logK_mid = (logK_lb + logK_ub) / 2.0
        # print(f'logK_mid = {logK_mid}')
        now_mass = Qprime_single_var_numerical(disttype, d, k, sigma, eta, beta, logK_mid, rad)
        if now_mass < Q:
            logK_ub = logK_mid
        else:
            logK_lb = logK_mid
...
"""
def check(disttype: str, r: float, d: int, k: int, sigma: float, eta: float, beta: float,
    ...
    method = {'gaussian': 'numerical',
              ...
              'gaussian-th': 'numerical',
              'exponential-general-gaussian-th': 'numerical', 
              'exponential-general-gaussian': 'numerical', 
              'exponential-standard-gaussian': 'numerical', 
              'exponential-standard-gaussian-th': 'numerical'}[disttype]
    ...
    if disttype.endswith('-th'):
        ...
        if isinstance(beta, float):
            # beta translation
            if disttype == 'exponential-general-gaussian-th':
                beta = Gamma((d - 2.0 * k) / eta).ppf(beta)
            if disttype == 'exponential-standard-gaussian-th':
                beta = Gamma(d / eta).ppf(beta)
    ...
    if method == 'numerical':
        ...
        if isinstance(beta, str) and beta.endswith('+'):
            ...
            logK1 = bin_search_K_on_P_numerical(disttype, d, k, sigma, eta, r, p1L)
            p1shift = Pshift_single_var_numerical(disttype, d, k, sigma, eta, logK1, r, limit=200)
            naive = True
            # print(f'info: logK1 = {logK1}, p1shift={p1shift}')

        else:
            # print('Not a naive case')
            
            logK1 = bin_search_K_on_P_numerical(disttype, d, k, sigma, eta, r, p1L)
            q1ideal = Q_single_var_numerical(disttype, d, k, sigma, eta, beta, logK1, r)
            ...
            if q1ideal >= q1R:
                p1 = p1L
                q1 = q1R
                # print(f'info: logK1 = {logK1}, q1ideal={q1ideal}')
            elif q1ideal >= q1L:
                p1shift = Pshift_single_var_numerical(disttype, d, k, sigma, eta, logK1, r, limit=200)
                naive = True
                # print(f"  Naive case A: {q1ideal} in [{q1L}, {q1R}]")
                # print(f'info: logK1 = {logK1}, q1ideal={q1ideal}, p1shift={p1shift}')
            else:
                q1ideal < q1L < q1R
                logK2 = bin_search_K_on_Qprime_numerical(disttype, d, k, sigma, eta, beta, r, q1L)
                p1ideal = Pprime_single_var_numerical(disttype, d, k, sigma, eta, beta, logK2, r)
                # print(f"  logK2 = {logK2}")
                # print(f"  p1ideal = {p1ideal}")
                if p1ideal < p1L:
                    p1 = p1L
                    q1 = q1L
                    # print(f'info: logK2 = {logK2}, p1ideal={p1ideal}')
                elif p1ideal > p1R:
                    p1 = p1R
                    q1 = q1L
                    # print(f'info: logK2 = {logK2}, p1ideal={p1ideal}')
                else:
                    logK3 = bin_search_K_on_P_numerical(disttype, d, k, sigma, eta, r, p1ideal, EPS=eps*1e-3)
                    print(f"  logK3 = {logK3}")
                    p1shift = Pshift_single_var_numerical(disttype, d, k, sigma, eta, logK3, r, limit=200)
                    naive = True
                    print(f"  Naive case B: {p1ideal} in [{p1L}, {p1R}]")
                    # print(f'info: logK2 = {logK2}, p1ideal={p1ideal}, logK3 = {logK3}, p1shift={p1shift}')
    else:
    ...
    if disttype == 'exponential-general-gaussian-th':
        ...
        def compute_Q_by_lamb12(now_lamb12):
            return nu * Gamma((d - 2 * k) / eta).expect(
            lambda t: Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                ((u2x(t, sigma, eta) + r) ** 2 - inv2_phig(u2x(t, sigma, eta), now_lamb12, k, sigma, eta))
                /
                (4.0 * u2x(t, sigma, eta) * r)
            ),
            lb=0., ub=beta
        ...
        """ enlarge lambda12 to tolerate numerical intergration error """
        """ an accelerated version, 100 * gap will be checked in advance, then gap"""
        ...
        while now_Ql + eps > q1:
            buf_lamL =  lamb12_L
            buf_qL = now_Ql
            lamb12_L -= 100 * lamb12_gap
            now_Ql = compute_Q_by_lamb12(lamb12_L)
        if (buf_lamL > 0 and buf_qL > 0):
            lamb12_L = buf_lamL
            now_Ql = buf_qL
        while now_Ql + eps > q1:
            lamb12_L -= lamb12_gap
        ...
        if p1it < eps:
            ...
            while lamb1_R - lamb1_L > eps:
                ...
                now_P1it = Gamma((d - 2 * k) / eta).expect(lambda t:
                    Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                        ((u2x(t, sigma, eta) + r) ** 2 - inv2_phig(u2x(t, sigma, eta), lamb1_mid, k, sigma, eta))
                        /
                        (4.0 * u2x(t, sigma, eta) * r)
                    ),
                    lb=beta, ub=np.inf)
        ...
        def u1(t):
            return Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                (min(u2x(beta, sigma, eta) ** 2, inv2_phig(u2x(t, sigma, eta), -lamb12_L, k, sigma, eta)) - (u2x(t, sigma, eta) - r) ** 2)
                /
                (4.0 * u2x(t, sigma, eta) * r )
            )
        # print(f'  u1(t)={Gamma((d - 2 * k) / eta).expect(lambda t: u1(t))}')
        ...
        def u2(t):
            if lamb1 < -range_width * 10.:
                ...
                return max(0.0, Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    (inv2_phig(u2x(t, sigma, eta), -lamb1_L, k, sigma, eta) - (u2x(t, sigma, eta) - r) ** 2)
                    /
                    (4.0  * r * u2x(t, sigma, eta))
                ) - Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    (u2x(beta, sigma, eta) ** 2 - (u2x(t, sigma, eta) - r) ** 2)
                    /
                    (4.0 * r * u2x(t, sigma, eta))
                ))
        # print(f'  u2(t)={Gamma((d - 2 * k) / eta).expect(lambda t: u2(t))}')

        # R lower bounds: for soundness
        R = Gamma((d - 2 * k) / eta).expect(lambda t: u1(t) + u2(t), lb=0., ub=np.inf)
        print(f'    R = {R}')
    ...
    if disttype == 'exponential-standard-gaussian-th':
        ...
        def compute_Q_by_lamb12(now_lamb12):
            return nu * Gamma(d / eta).expect(lambda t: get_Beta_val_forQ(t, now_lamb12), lb=0., ub=beta)
        ...
        # print(f'Q calculated, lamb12 = {lamb12_L}, now Q is {now_Q}')
        """ enlarge lambda12 to tolerate numerical intergration error """
        """ an accelerated version, 100 * gap will be checked in advance, then gap"""
        lamb12_gap = (lamb12_R - lamb12_L) / 2.
        now_Ql = compute_Q_by_lamb12(lamb12_L)
        ...
        if p1it < eps:
            ...
            while lamb1_R - lamb1_L > eps:
                ...
                def compute_H1low_by_lamb12(now_lamb1):
                    return Gamma(d / eta).expect(lambda t: get_Beta_val_forH(t, now_lamb1), lb=0., ub=beta)

                # print(f'now lamb1 is {lamb1_mid}')
                # print(f'infty {Int_for_gamma(get_Beta_val_forH, d, eta, lamb1_mid)}, in beta {compute_H1_by_lamb12(lamb1_mid)}')
                now_P1it = Int_for_gamma(get_Beta_val_forH, d, eta, lamb1_mid) - compute_H1low_by_lamb12(lamb1_mid)
            ...
            lamb1 = (lamb1_L + lamb1_R) / 2.0
            # print(f'h calculated, lamb1 = {lamb1}')
            # by Theorem 5, we only need lamb1_L and lamb12_L to compute R

        """ integrate R """
        def get_Beta_val_for_u1(u, logK):
            if u + logK >= 1e-10 and u >= 1e-10:
                ans = Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    ( min(u2x(beta, sigma, eta) ** 2, inv2_phis(u2x(u, sigma, eta), -logK, sigma, eta)) - (u2x(u, sigma, eta) - r) ** 2)
                    /
                (4.0 * u2x(u, sigma, eta) * r )
            )
            else:
        ...
        def get_Beta_val_for_u2(u, logK):
            if logK < -range_width * 10.:
                # lamb1 = 0
                return 0.0

            if u + logK >= 1e-10 and u >= 1e-10:
                ans = max(0.0, Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    (inv2_phis(u2x(u, sigma, eta), -logK, sigma, eta) - (u2x(u, sigma, eta) - r) ** 2)
                    /
                    (4.0 * r * u2x(u, sigma, eta))
                ) - Beta((d - 1.0) / 2.0, (d - 1.0) / 2.0).cdf(
                    (u2x(beta, sigma, eta) ** 2 - (u2x(u, sigma, eta) - r) ** 2)
                    /
                    (4.0 * r * u2x(u, sigma, eta))
                ))
                return ans
        ...
        R =  Int_for_gamma(get_Beta_val_for_u1, d, eta, lamb12_L) +  Int_for_gamma(get_Beta_val_for_u2, d, eta, lamb1_L)
